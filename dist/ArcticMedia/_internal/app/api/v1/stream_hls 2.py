...
async def get_master_playlist(
    ...
    # playlist stream-copies HEVC when no subtitle burn-in is requested
    # (see get_media_playlist); advertising avc1 for HEVC segments makes
    # strict clients (AVPlayer) reject or mis-decode the stream.
    file_info = await asyncio.to_thread(get_detailed_media_info, mf.path)
    probe_vcodec = (file_info.get("vcodec") or "").lower()
    if sidx is None and probe_vcodec in ("hevc", "h265"):
    ...
    lines = [
        "#EXTM3U",
        "#EXT-X-VERSION:3",
        f"#EXT-X-STREAM-INF:BANDWIDTH=2500000,CODECS=\"{codecs}\"",
        playlist_url,
    ]
    return Response(
        "\n".join(lines),
