...
function BuildHlsUrl(serverUrl as string, token as string, mediaId as integer) as string
    ...
    ' The server knows the source codec, so it transcodes only when the source
    ' is actually HEVC and leaves the resolution alone. No height cap: that was
    ' a workaround for servers predating the hevc parameter, and it downscaled
    ' H.264 that would otherwise have been copied untouched.
    if not DeviceCanDecodeHevc() then url = url + "&hevc=0"
    return url
end function
